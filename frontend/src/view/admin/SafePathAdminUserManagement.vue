    ...
    <v-container fluid class="pa-4 pa-md-6">
        <v-card id="users" rounded="lg" elevation="2">
            ...
            <v-card-text>
                ...
                <v-data-table-server :headers="userHeaders" :items="users" :items-length="totalUsers"
                    :loading="usersLoading" :items-per-page="itemsPerPage" :page="page"
                    ...
                    <template #no-data>
                        <div class="text-center text-medium-emphasis py-6">No users found.</div>
                    </template>
                </v-data-table-server>
            </v-card-text>
        </v-card>
...
const itemsPerPage = ref(10);   // replaces the `const pageSize = 20`

// Fetch one page from Keycloak using the current page + itemsPerPage.
const fetchUsers = async () => {
    ...
    try {
        const { data } = await adminService.listUsers({
            ...
            first: (page.value - 1) * itemsPerPage.value,
...
// The table emits { page, itemsPerPage, sortBy, ... } — sync then reload.
const onOptionsUpdate = ({ page: p, itemsPerPage: ipp }) => {
    page.value = p;
